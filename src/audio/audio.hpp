...
struct meta_data {
  meta_data(i32 chans, i32 sr, i32 file_samples, i32 file_bytes)
      : channels(chans), sample_rate(sr), samples(file_samples),
        bytes(file_bytes), position(0) {}
  i32 channels;
  i32 sample_rate;
  u32 samples;
  u32 bytes;
...
struct audio_streambuffer {
  audio_streambuffer()
      : stream(nullptr), output_spec({SDL_AUDIO_F32, 2, 44100}), device_id(0) {}
  ...
  bool spec_compare(const SDL_AudioSpec *original,
