...
audio_data read_file(file_data file){
  ...
  std::vector<f32> buffer = std::vector<f32>(samples);
  if(sf_read_float(file.open, buffer.data(), samples) < 0){
    log_write_str("Error while reading audio chunks:", sf_strerror(file.open));
    sf_close(file.open);
    return audio_data(vecf32(), vecf64(), meta_data(0, 0, 0, 0), false);
  }
  sf_close(file.open);
  return audio_data(buffer, vecf64(FFT_SIZE), meta_data(file.channels, file.sample_rate, samples, bytes), true);
}
